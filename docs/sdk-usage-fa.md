...
2. همان نمونهٔ استراتژی را در سشن امروز ۱۲:۰۰–۱۷:۰۰ تهران، با پول هر **یک ثانیه**، پیپر کنید.
...
| مسیر | رفتار |
| --- | --- |
| `month_backtest(..., grain="1s", fill_session=True)` | روی هر روز که داده دارد، از ۱۲:۰۰ تا ۱۷:۰۰ تهران هر ثانیه یک اسنپ‌شات (حدود ۱۸۰۰۱ نقطه در روز سشن). شب و تعطیل پر نمی‌شود. |
| `month_backtest(..., fill_session=False)` | فقط ثانیه‌های بین اولین و آخرین بار همان روز سشن، با ffill. |
| `iran_session_live(..., poll_seconds=1.0)` | پول زنده هر ۱٫۰ ثانیه تا ۱۷:۰۰. پیش‌فرض `LiveDataProvider` هم `poll_seconds=1.0` و `bar_grain="1s"` است. |

`window_days` تعداد تیک نیست؛ پنجرهٔ **تقویمی** روی پریمیوم‌های زمان‌دار ۱ ثانیه‌ای است.

`fill_session=True` روی ماه کامل سنگین است (۳۰ نماد × حدود ۲۰ روز سشن × ۱۸۰۰۱ ثانیه). برای آزمایش محلی `GOLDARB_FILL_SESSION=0` روی مثال ماهانه.
...
`simulate_iran_session_1s.py` تا ۱۲:۰۰ تهران صبر می‌کند و تا ۱۷:۰۰ هر ثانیه پول می‌کند.
