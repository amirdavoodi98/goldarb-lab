...
def test_market_order_fee_portfolio_and_restart(tmp_path):
    ...
    with LocalSimulator(path) as sim:
        account = sim.create_account(
            initial_cash="10000", fee_rate="0.001", allow_short=False
        )
...
def test_short_policy_cancel_and_realized_pnl(tmp_path):
    with LocalSimulator(tmp_path / "sim.db") as sim:
        ...
        rejected = sim.submit_order(
            long_only.id, symbol="طلا", side="SELL", quantity="1"
        )
        assert rejected.status == OrderStatus.REJECTED

        sell = sim.submit_order(
            shortable.id, symbol="طلا", side="SELL", quantity="2"
        )
        assert sell.status == OrderStatus.FILLED
        sim.feed(snapshot("tick-2", minute=1, bid="89", ask="90"))
        cover = sim.submit_order(
            shortable.id, symbol="طلا", side="BUY", quantity="2"
        )
...
def test_orders_share_depth_within_same_market_event(tmp_path):
    with LocalSimulator(tmp_path / "sim.db") as sim:
        ...
        first = sim.submit_order(
            account.id, symbol="طلا", side="BUY", quantity="1"
        )
